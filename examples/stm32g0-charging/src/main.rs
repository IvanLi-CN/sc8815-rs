...
async fn main(_spawner: Spawner) {
    ...
    info!("SC8815 configured as battery charger: 4S LiFePO4, 1A charging current");
    info!("Battery voltage limit: 18.2V (4.55V per cell)");
    ...
    loop {
        ...
        match sc8815_driver.get_adc_measurements_with_config(&config.current_limits).await {
            Ok(measurements) => {
                ...
                if let (Ok(ibus_high), Ok(ibus_low)) = (
                    sc8815_driver.read_register(sc8815::registers::Register::IbusValue).await,
                    sc8815_driver.read_register(sc8815::registers::Register::IbusValue2).await,
                ) {
